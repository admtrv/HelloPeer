...
void Node::fsm_process(unsigned char* buff, size_t length)
{
    ...
    if (packet.header.flags == TCU_HDR_FLAG_SYN)
    {
        process_tcu_conn_req(packet);
    }
    else if (packet.header.flags == (TCU_HDR_FLAG_SYN | TCU_HDR_FLAG_ACK))
    {
        process_tcu_conn_ack(packet);
    }
    else if (packet.header.flags == TCU_HDR_FLAG_FIN)
    {
        process_tcu_disconn_req(packet);
    }
    else if (packet.header.flags == (TCU_HDR_FLAG_FIN | TCU_HDR_FLAG_ACK))
    {
